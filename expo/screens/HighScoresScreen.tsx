...
export function HighScoresScreen({ game, title, onBack, backVariant = 'ghost' }: { game: GameId; title: string; onBack: () => void; backVariant?: 'primary' | 'ghost' | 'danger' }): React.ReactElement {
  const [scores, setScores] = useState<HighScoreEntry[]>([]);
  useEffect(() => { void getHighScores(game).then(setScores); }, [game]);
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{title}</Text>
      <Text style={styles.subtitle}>TOP 10 PILOTS</Text>
      <View style={styles.board}>
        {scores.length === 0 ? <Text style={styles.empty}>Aucun score pour l’instant.</Text> : scores.map((s, i) => (
...
const styles = StyleSheet.create({
  ...
  board: { width: '100%', maxWidth: 430, borderRadius: 24, padding: 18, backgroundColor: 'rgba(0,0,0,0.28)', borderWidth: 1, borderColor: 'rgba(98,246,255,0.3)', marginBottom: 22, minHeight: 320 },
  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
